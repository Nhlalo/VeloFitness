...
export default function SetPassword() {
  ...
  const previousUrl = location?.state?.from == "jointoday";

  // Show payment success popup if previousUrl is true
  useEffect(() => {
    if (previousUrl) {
  ...
  return (
    <>
      ...
      {showPaymentPopup && (
        <div className="animate-in slide-in-from-top fade-in fixed top-4 left-1/2 z-50 w-auto max-w-md -translate-x-1/2 duration-300">
          <div className="flex items-center gap-3 rounded-lg border border-green-500/30 bg-green-500/10 px-4 py-3 shadow-lg backdrop-blur-sm">
            <CheckCircle2 className="h-5 w-5 shrink-0 text-green-500" />
            <p className="text-sm font-medium text-green-500">
              Payment successful! Your account has been verified.
            </p>
            <button
              onClick={() => setShowPaymentPopup(false)}
              className="ml-2 shrink-0 text-green-500/70 transition-colors hover:text-green-500"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
